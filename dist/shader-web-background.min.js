...
f.location,b)};this.j=()=>{var f=c.A,h=a.g;h.bindBuffer(h.ARRAY_BUFFER,a.l);h.enableVertexAttribArray(f);
h.vertexAttribPointer(f,2,h.FLOAT,!1,0,0);h.drawArrays(h.TRIANGLE_STRIP,0,4);h.disableVertexAttribArray(f);
h.bindBuffer(h.ARRAY_BUFFER,null);f=a.g;for(h=0;h<a.h;h++)f.activeTexture(f.TEXTURE0+
h),f.bindTexture(f.TEXTURE_2D,null);a.h=0}}}
...
function I(a,b,c,d,k,f){function h(e,m){try{{var n=q;const p=n.g,v=J(n,e,p.VERTEX_SHADER,"attribute vec2 V;void main(){gl_Position=vec4(V,0,1);}"),
...
}}const q=G(a),P=q.g,w=[],g={canvas:a,width:0,height:0,cssPixelRatio:0,cssWidth:0,
...
.5,u:()=>g.cssWidth!==a.clientWidth||g.cssHeight!==a.clientHeight?(g.resize(),!0):
...
e;g.cssWidth=m;g.cssHeight=n;q.g.viewport(0,0,q.canvas.width,q.canvas.height);for(const v of w)v.g.m(l,
...
b[e].texture||r;g.buffers[e]=L(q,()=>{l(P,g)})}const m=M(q,h(e,D(e)),g.buffers[e]),
...
t.join(", "));t=m.o.map(l=>({location:l.location,v:n[l.name]}));w.push(new F(q,g,
m,t))}const C=()=>{g.u()&&d&&d(g.width,g.height,g);k&&k(g);for(const e of w)e.g.j(e.h,
e.j);f&&f(g);requestAnimationFrame(C)};E("load",()=>{g.resize();c&&c(g);d&&d(g.width,
...
h.name)})}return{A:d.getAttribLocation(b,"V"),o:a,m:c?(f,h)=>c.m(f,h):()=>{},j:(f,
h)=>{d.useProgram(b);f();c?(f=c.g,c.g=c.h,c.h=f,c.j(h)):h()}}}
